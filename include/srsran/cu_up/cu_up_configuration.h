...
struct network_interface_config {
  ...
  /// Local IP address to bind for connection from DU to receive uplink user-plane traffic.
  std::string m1_bind_addr = "127.0.2.1";

  /// External IP address that is advertised to receive MR-U packets from the DU.
  /// It defaults to \c m1_bind_addr but may differ in case the CU-UP is behind a NAT.
  std::string m1_ext_addr = "auto";

  /// Local port to bind for connection from DU to receive uplink user-plane traffic.
  int m1_bind_port = M1_GTPU_PORT;
};
...
struct cu_up_configuration {
  ...
  // m1_cu_up_gateway*      m1_gateway = nullptr;
...
struct formatter<srsran::srs_cu_up::network_interface_config> {
  ...
  auto format(const srsran::srs_cu_up::network_interface_config& cfg, FormatContext& ctx)
  {
    return format_to(ctx.out(),
                     "upf_port={}, n3_bind_addr={}, n3_bind_port={}, f1u_bind_addr={}, f1u_bind_port={}, m1_bind_addr={}, m1_bind_port={}",
                     cfg.upf_port,
                     cfg.n3_bind_addr,
                     cfg.n3_bind_port,
                     cfg.f1u_bind_addr,
