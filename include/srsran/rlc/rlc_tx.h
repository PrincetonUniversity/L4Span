...
/// This interface represents the data entry point of the transmitting side of a RLC entity.
/// The MARK layer may use this call to pass some information, but at this point, this is just a dummy handler.
class rlc_tx_m1_data_interface
{
public:
  rlc_tx_m1_data_interface()                                                     = default;
  virtual ~rlc_tx_m1_data_interface()                                            = default;
  rlc_tx_m1_data_interface(const rlc_tx_m1_data_interface&)             = delete;
  rlc_tx_m1_data_interface& operator=(const rlc_tx_m1_data_interface&)  = delete;
  rlc_tx_m1_data_interface(const rlc_tx_m1_data_interface&&)            = delete;
  rlc_tx_m1_data_interface& operator=(const rlc_tx_m1_data_interface&&) = delete;

  /// \brief Interface for higher layers to pass SDUs into RLC
  /// \param sdu_buf SDU to be handled
  virtual void handle_m1_sdu(byte_buffer sdu_buf) = 0;
};
...
// /// This interface represents the data M1 layer that the TX RLC bearer must notify on some point.
// ///
// class rlc_tx_m1_data_notifier
// {
// public:
//   virtual ~rlc_tx_m1_data_notifier() = default;
