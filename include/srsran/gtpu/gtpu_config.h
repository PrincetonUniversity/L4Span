...
constexpr unsigned M1_GTPU_PORT = 8013;
...
/// \brief Configurable parameters for GTP-U NR-U tunnels
struct gtpu_tunnel_mru_config {
  struct gtpu_tunnel_mru_rx_config {
    gtpu_teid_t local_teid;
  } rx;
  struct gtpu_tunnel_mru_tx_config {
    gtpu_teid_t peer_teid;
    std::string peer_addr;
    uint16_t    peer_port;
  } tx;
...
// GTP-U NR-U RX config
template <>
struct formatter<srsran::gtpu_tunnel_mru_config::gtpu_tunnel_mru_rx_config> {
  template <typename ParseContext>
  auto parse(ParseContext& ctx) -> decltype(ctx.begin())
  {
    return ctx.begin();
  ...
  template <typename FormatContext>
  auto format(const srsran::gtpu_tunnel_mru_config::gtpu_tunnel_mru_rx_config& cfg, FormatContext& ctx)
      -> decltype(std::declval<FormatContext>().out())
  {
    return format_to(ctx.out(), "local_teid={}", cfg.local_teid);
  }
...
// GTP-U NR-U TX config
template <>
struct formatter<srsran::gtpu_tunnel_mru_config::gtpu_tunnel_mru_tx_config> {
  template <typename ParseContext>
  auto parse(ParseContext& ctx) -> decltype(ctx.begin())
  {
    return ctx.begin();
  ...
  template <typename FormatContext>
  auto format(const srsran::gtpu_tunnel_mru_config::gtpu_tunnel_mru_tx_config& cfg, FormatContext& ctx)
      -> decltype(std::declval<FormatContext>().out())
  {
    return format_to(ctx.out(), "peer_teid={} peer_addr={} peer_port={}", cfg.peer_teid, cfg.peer_addr, cfg.peer_port);
  }
...
// GTP-U NR-U config
template <>
struct formatter<srsran::gtpu_tunnel_mru_config> {
  template <typename ParseContext>
  auto parse(ParseContext& ctx) -> decltype(ctx.begin())
  {
    return ctx.begin();
  ...
  template <typename FormatContext>
  auto format(const srsran::gtpu_tunnel_mru_config& cfg, FormatContext& ctx)
      -> decltype(std::declval<FormatContext>().out())
  {
    return format_to(ctx.out(), "{} {}", cfg.rx, cfg.tx);
  }
