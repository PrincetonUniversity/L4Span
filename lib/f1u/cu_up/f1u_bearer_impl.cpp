...
f1u_bearer_impl::f1u_bearer_impl(uint32_t                       ue_index_,
                                 drb_id_t                       drb_id_,
                                 ...
                                 f1u_rx_delivery_notifier_to_mark& rx_delivery_notifier_to_mark_,
  ...
  rx_delivery_notifier_to_mark(rx_delivery_notifier_to_mark_),
...
void f1u_bearer_impl::handle_pdu_to_mark(nru_ul_message msg)
{
  auto fn = [this, m = std::move(msg)]() mutable { handle_pdu_to_mark_impl(std::move(m)); };

  if (not ul_mark_exec.defer(std::move(fn))) {
    if (!cfg.warn_on_drop) {
      logger.log_info("Dropped F1-U PDU to MARK, task queue is full.");
    } else {
      logger.log_warning("Dropped F1-U PDU to MARK, task queue is full.");
    }
  }
}
...
void f1u_bearer_impl::handle_pdu_to_mark_impl(nru_ul_message msg)
{
  logger.log_debug("F1-U bearer received PDU to MARK");
  mark_utils::delivery_status_feedback feedback = {};

  if (msg.data_delivery_status.has_value()) {
    nru_dl_data_delivery_status& status = msg.data_delivery_status.value();
    if (status.highest_transmitted_pdcp_sn.has_value()) {
      uint32_t pdcp_sn = status.highest_transmitted_pdcp_sn.value();
      feedback.highest_pdcp_sn_transmitted = pdcp_sn;
      logger.log_debug("Notifying highest transmitted pdcp_sn={} to MARK", pdcp_sn);
    }
    // Highest successfully delivered PDCP SN
    if (status.highest_delivered_pdcp_sn.has_value()) {
      uint32_t pdcp_sn = status.highest_delivered_pdcp_sn.value();
      feedback.highest_pdcp_sn_delivered = pdcp_sn;
      logger.log_debug("Notifying highest successfully delivered pdcp_sn={} to MARK", pdcp_sn);
    }
    // Highest retransmitted PDCP SN
    if (status.highest_retransmitted_pdcp_sn.has_value()) {
      uint32_t pdcp_sn = status.highest_retransmitted_pdcp_sn.value();
      feedback.highest_pdcp_sn_retransmitted = pdcp_sn;
      logger.log_debug("Notifying highest retransmitted pdcp_sn={} to MARK", pdcp_sn);
    }
    // Highest successfully delivered retransmitted PDCP SN
    if (status.highest_delivered_retransmitted_pdcp_sn.has_value()) {
      uint32_t pdcp_sn = status.highest_delivered_retransmitted_pdcp_sn.value();
      feedback.highest_pdcp_sn_delivered_retransmitted = pdcp_sn;
      logger.log_debug("Notifying highest successfully delivered retransmitted pdcp_sn={} to MARK", pdcp_sn);
    }
  }

  rx_delivery_notifier_to_mark.on_delivery_status(feedback, drb_id);
}
