...
std::unique_ptr<m1_du_gateway_bearer>
m1_split_connector::create_du_bearer(uint32_t                                  ue_index,
                                     drb_id_t                                  drb_id,
                                     ...
                                     task_executor&                            ue_executor)
{
  ...
  std::unique_ptr<m1_split_gateway_du_bearer> du_bearer = std::make_unique<m1_split_gateway_du_bearer>(
...
void m1_split_connector::remove_du_bearer(const up_transport_layer_info& dl_up_tnl_info)
{
  m1_split_gateway_du_bearer* du_bearer = nullptr;
  {
    std::unique_lock<std::mutex> lock(map_mutex);
  ...
  du_bearer->gtpu_to_m1_adapter.disconnect();
  ...
  logger_du.debug("Removed CU M1 bearer with UL GTP Tunnel={}.", dl_up_tnl_info);
}

expected<std::string> m1_split_connector::get_du_bind_address(gnb_du_id_t gnb_du_id) const
{
