...
public:
  ue_executor_mapper_impl(task_executor& ctrl_exec_,
                          ...
                          task_executor& ul_mark_exec_,
                          // task_executor& ul_m1_exec_,
                          // task_executor& dl_m1_exec_,
                          task_executor& crypto_exec_,
                          task_executor& main_exec_) :
    ctrl_exec(ctrl_exec_), ul_exec(ul_exec_), dl_exec(dl_exec_), 
    ul_mark_exec(ul_mark_exec_),
    // ul_m1_exec(ul_m1_exec_), 
    // dl_m1_exec(dl_m1_exec_), 
    crypto_exec(crypto_exec_), main_exec(main_exec_)
  {
  }
  ...
  task_executor& ul_pdu_mark_executor() override { return *ul_mark_exec; }

  // task_executor& ul_m1_pdu_executor() override { return *ul_m1_exec; }
  // task_executor& dl_m1_pdu_executor() override { return *dl_m1_exec; }
...
private:
  ...
  std::optional<cancellable_task_executor> ul_mark_exec;
  // std::optional<cancellable_task_executor> ul_m1_exec;
  // std::optional<cancellable_task_executor> dl_m1_exec;
...
struct base_cu_up_executor_pool_config {
  ...
  span<task_executor*> ul_mark_executors;
  // span<task_executor*> dl_m1_executors;
  // span<task_executor*> ul_m1_executors;
...
public:
  round_robin_cu_up_exec_pool(base_cu_up_executor_pool_config config) : main_exec(config.main_exec)
  {
    ...
    if (config.ul_mark_executors.empty()) {
      config.ul_mark_executors = config.ctrl_executors;
    } else {
      srsran_assert(config.ul_mark_executors.size() == config.ctrl_executors.size(),
                    "If specified, the number of UL MARK executors must be equal to the number of control executors");
    }

    for (unsigned i = 0; i != config.ctrl_executors.size(); ++i) {
      execs.emplace_back(
          *config.ctrl_executors[i], *config.ul_executors[i], *config.dl_executors[i], 
          *config.ul_mark_executors[i],
          // *config.ul_m1_executors[i], *config.dl_m1_executors[i], 
          config.crypto_exec);
    }
  }
  ...
  std::unique_ptr<ue_executor_mapper> create_ue_executor_mapper()
  {
    ...
    return std::make_unique<ue_executor_mapper_impl>(
        ctxt.ctrl_exec, ctxt.ul_exec, ctxt.dl_exec, ctxt.ul_mark_exec,
        // ctxt.ul_m1_exec, ctxt.dl_m1_exec, 
        ctxt.crypto_exec, main_exec);
  }
...
private:
  struct ue_executor_context {
    ...
    task_executor& ul_mark_exec;
    // task_executor& ul_m1_exec;
    // task_executor& dl_m1_exec;
    task_executor& crypto_exec;
    ...
    ue_executor_context(task_executor& ctrl_exec_,
      ...
      ctrl_exec(ctrl_exec_), ul_exec(ul_exec_), dl_exec(dl_exec_), 
      ul_mark_exec(ul_mark_exec_),
      // ul_m1_exec(ul_exec_), dl_m1_exec(dl_exec_), 
...
public:
  ...
  // task_executor& io_ul_m1_executor() override { return *io_ul_m1_exec; }
...
private:
  ...
  base_cu_up_executor_pool_config create_strands(const strand_based_executor_config& config)
  {
    ...
    if (config.dedicated_io_strand) {
      ...
      // io_ul_m1_strand.emplace<io_dedicated_strand_type>(&cu_up_strand, config.default_task_queue_size);
      // io_ul_m1_exec = &std::get<io_dedicated_strand_type>(io_ul_m1_strand);
    } else {
      io_ul_strand.emplace<inline_task_executor>();
      io_ul_exec = &std::get<inline_task_executor>(io_ul_strand);
      // io_ul_m1_strand.emplace<inline_task_executor>();
      // io_ul_m1_exec = &std::get<inline_task_executor>(io_ul_m1_strand);
    }
    ...
    ue_ul_mark_execs.resize(config.max_nof_ue_strands);
    // ue_dl_m1_execs.resize(config.max_nof_ue_strands);
    // ue_ul_m1_execs.resize(config.max_nof_ue_strands);
    ue_dl_execs.resize(config.max_nof_ue_strands);
    std::array<concurrent_queue_params, 4> ue_queue_params = {qparams, data_qparams, data_qparams, data_qparams};
    for (unsigned i = 0; i != config.max_nof_ue_strands; ++i) {
      ...
      srsran_assert(execs.size() == 4, "Four executors should have been created for the four priorities");
      ue_ctrl_execs[i] = &execs[0];
      ue_ul_execs[i]   = &execs[1];
      ue_dl_execs[i]   = &execs[2];
      ue_ul_mark_execs[i] = &execs[3];
    }

    return base_cu_up_executor_pool_config{
        cu_up_strand, ue_dl_execs, ue_ul_execs, 
        ue_ul_mark_execs,
        // ue_dl_m1_execs, 
        // ue_ul_m1_execs, 
        ue_ctrl_execs, 
        config.worker_pool_executor};
  }
  ...
  // IO executor for the M1 interface
  // std::variant<inline_task_executor, io_dedicated_strand_type> io_ul_m1_strand;
  // task_executor*                                               io_ul_m1_exec;
  ...
  std::vector<task_executor*>                  ue_ul_mark_execs;
  // std::vector<task_executor*>                  ue_ul_m1_execs;
  // std::vector<task_executor*>                  ue_dl_m1_execs;
