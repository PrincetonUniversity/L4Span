...
pdu_session_manager_impl::pdu_session_manager_impl(ue_index_t                                       ue_index_,
  ...
  // m1_teid_allocator(m1_teid_allocator_),
  gtpu_rx_demux(gtpu_rx_demux_),
  ue_dl_exec(ue_dl_exec_),
  ue_ul_exec(ue_ul_exec_),
  ue_ul_mark_exec(ue_ul_mark_exec_),
  // ue_m1_ul_exec(ue_m1_ul_exec_),
...
pdu_session_setup_result pdu_session_manager_impl::setup_pdu_session(const e1ap_pdu_session_res_to_setup_item& session)
{
  ...
  sdap_entity_creation_message sdap_msg = {ue_index, session.pdu_session_id, &new_session->sdap_to_mark_adapter};
  ...
  /* Change to gtpu_to_mark_adapter interface */
  msg.rx_lower                         = &new_session->gtpu_to_mark_adapter; 
  ...
  // Create MARK entity
  mark_entity_creation_message mark_msg = {ue_index, session.pdu_session_id, 
    &new_session->mark_to_gtpu_adapter, session.drb_to_setup_list_ng_ran.size()};
  new_session->mark                     = create_mark(mark_msg);

  // Create mark notifier
  new_session->mark->create_rx();
  new_session->mark->create_tx(new_session->mark_to_sdap_adapter);
  ...
  // Connect adapters of GTPU and MARK
  new_session->mark_to_gtpu_adapter.connect_gtpu(*new_session->gtpu->get_tx_lower_layer_interface());
  new_session->gtpu_to_mark_adapter.connect_mark(new_session->mark->get_mark_tx_sdu_handler());

  // Connect adapters of MARK and SDAP
  new_session->mark_to_sdap_adapter.connect_sdap(new_session->sdap->get_sdap_tx_sdu_handler());
  new_session->sdap_to_mark_adapter.connect_mark(new_session->mark->get_mark_rx_pdu_handler());
                                                                    ...
                                                                    const e1ap_drb_to_setup_item_ng_ran& drb_to_setup)
{
  ...
  // Add drb id in the MARK entity
  new_session.mark->add_drb(drb_to_setup.drb_id, drb_to_setup.pdcp_cfg.rlc_mod);
  ...
  for (const auto& qos_flow_info : drb_to_setup.qos_flow_info_to_be_setup) {
    ...
    new_session.mark->add_mapping(qos_flow_info.qos_flow_id, drb_to_setup.drb_id);
  ...
  // Connect F1-U to the MARK layer
  new_drb->f1u_to_mark_adapter.connect_mark(new_session.mark->get_mark_rx_lower_interface(), 
                                            new_session.mark->get_mark_tx_lower_interface());
  new_drb->f1u_gateway_rx_to_nru_adapter.connect_nru_mark_bearer(new_drb->f1u->get_rx_pdu_mark_handler());

  new_session.mark->set_pdcp_sn_size(drb_to_setup.drb_id, pdcp_sn_size_to_uint(drb_to_setup.pdcp_cfg.pdcp_sn_size_dl));

  // Add result
  drb_result.success = true;  

  // Create MARK context for M1 connection
  // new_session.mark_ctxs.emplace(drb_to_setup.drb_id, std::make_unique<mark_context>(drb_to_setup.drb_id));
  // mark_context* new_mark_ctx = new_session.mark_ctxs.at(drb_to_setup.drb_id).get();

  // Use the same teid as the F1-U interfaces, incurring less burden on a new TEID exchange.
  // gtpu_teid_t m1_ul_teid = f1u_ul_teid; 
  // up_transport_layer_info m1_ul_tunnel_addr(transport_layer_address::create_from_string(net_config.m1_bind_addr),
  //                                            m1_ul_teid);
  // new_mark_ctx->m1_gw_bearer = m1_gw.create_cu_bearer(ue_index, 
  //                                                     drb_to_setup.drb_id,
  //                                                     new_mark_ctx->m1_cfg, 
  //                                                     m1_ul_tunnel_addr, 
  //                                                     new_mark_ctx->m1_gateway_rx_to_mru_adapter,
  //                                                     ue_m1_ul_exec);

  // new_mark_ctx->m1 = srs_cu_up::create_m1_bearer(ue_index,
  //                                                new_mark_ctx->drb_id,
  //                                                m1_ul_tunnel_addr,
  //                                                new_mark_ctx->m1_cfg,
  //                                                *new_mark_ctx->m1_gw_bearer,
  //                                                new_mark_ctx->m1_to_mark_adapter,
  //                                                ue_dl_timer_factory,
  //                                                ue_inactivity_timer,
  //                                                ue_m1_ul_exec);

  /// TODO_HW: Connect MARK to NRU

  // new_mark_ctx->m1_ul_teid = m1_ul_teid;

  // Connect M1 GW bearer RX adapter to MR-U bearer
  // new_mark_ctx->m1_gateway_rx_to_mru_adapter.connect_mru_bearer(new_mark_ctx->m1->get_rx_pdu_handler());

  // Connect M1's "M1->MARK adapter" directly to MARK
  // Now it's connecting to a single MARK rx interface, maybe we should create multiple?
  // new_mark_ctx->m1_to_mark_adapter.connect_mark(new_session.mark->get_mark_rx_m1_interface(), 
  //                                               new_session.mark->get_mark_tx_m1_interface());
  // new_mark_ctx->mark_to_m1_adapter.connect_m1(new_mark_ctx->m1->get_tx_sdu_handler());
  ...
  for (const auto& drb_to_mod : session.drb_to_modify_list_ng_ran) {
    ...
    if (new_ul_tnl_info_required) {
      ...
      drb->f1u_gw_bearer = f1u_gw.create_cu_bearer(
          ue_index, drb->drb_id, drb->f1u_cfg, f1u_ul_tunnel_addr, drb->f1u_gateway_rx_to_nru_adapter, ue_ul_exec, ue_ul_exec);
...
void pdu_session_manager_impl::update_ue_number(size_t ue_number)
{
  for (auto & pdu_session : pdu_sessions) {
    pdu_session.second.get()->mark.get()->nof_ue = ue_number;
  }
}
