...
cu_up::cu_up(const cu_up_configuration& config_) : cfg(config_), main_ctrl_loop(128)
{
  ...
  // Create M1 TEID allocator
  // gtpu_allocator_creation_request m1_alloc_msg = {};
  // m1_alloc_msg.max_nof_teids                   = MAX_NOF_UES * MAX_NOF_PDU_SESSIONS;
  // m1_teid_allocator                            = create_gtpu_allocator(m1_alloc_msg);
  ...
  cu_up_mng = std::make_unique<cu_up_manager_impl>(
      cfg, *e1ap, gtpu_gw_adapter, *ngu_demux, 
      *n3_teid_allocator, *f1u_teid_allocator//, *m1_teid_allocator
      );
