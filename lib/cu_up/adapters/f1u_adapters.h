...
#include "srsran/f1u/cu_up/f1u_rx_delivery_notifier_to_mark.h"
...
public:
  ...
  void connect_nru_mark_bearer(f1u_rx_pdu_mark_handler& f1u_mark_handler_) { f1u_mark_handler = &f1u_mark_handler_; }
  ...
  void on_new_pdu(nru_ul_message msg) override
  {
    ...
    f1u_mark_handler->handle_pdu_to_mark(std::move(msg));
  }

private:
  f1u_rx_pdu_handler* f1u_handler = nullptr;
  f1u_rx_pdu_mark_handler* f1u_mark_handler = nullptr;
};

/// Adapter between F1-U and MARK
class f1u_mark_adapter final : public f1u_rx_delivery_notifier_to_mark
{
public:
  void connect_mark(mark_rx_lower_interface& pdcp_rx_handler_, mark_tx_lower_interface& pdcp_tx_handler_)
  {
    mark_rx_handler = &pdcp_rx_handler_;
    mark_tx_handler = &pdcp_tx_handler_;
  }

  void on_delivery_status(mark_utils::delivery_status_feedback feedback, drb_id_t drb_id_) override
  {
    mark_tx_handler->handle_feedback(feedback, drb_id_);
  }

  // void on_transmit_notification(uint32_t highest_pdcp_sn) override
  // {
  //   // TODO_HW: Add or change the functions
  //   // mark_tx_handler->handle_transmit_notification(highest_pdcp_sn);
  // }

  // void on_delivery_notification(uint32_t highest_pdcp_sn, drb_id_t drb_id_) override
  // {
  //   byte_buffer buf = {};
  //   bool res = buf.append(0x3);
  //   if (res){
  //     mark_tx_handler->handle_feedback(std::move(buf), drb_id_);
  //   }
  // }
...
private:
  mark_rx_lower_interface* mark_rx_handler = nullptr;
  mark_tx_lower_interface* mark_tx_handler = nullptr;
};
