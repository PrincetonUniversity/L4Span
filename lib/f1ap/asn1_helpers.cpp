...
static std::string derive_m1_from_f1u(transport_layer_address f1u_addr) {
  // only support IPv4 address format
  std::string f1u_addr_str = f1u_addr.to_string();
  std::vector<size_t> dot_pos;
  dot_pos.reserve(3);
  for (size_t i = 0; i < f1u_addr_str.length(); i++){
    if (f1u_addr_str[i] == '.') {
      dot_pos.push_back(i);
    }
  }
  std::string f1u_head = f1u_addr_str.substr(0, dot_pos[1]+1);
  std::string f1u_tail = f1u_addr_str.substr(dot_pos[2], f1u_addr_str.length()-dot_pos[2]);
  int sub_net = std::stoi(f1u_addr_str.substr(dot_pos[1]+1, dot_pos[2]-dot_pos[1]-1)) + 1;

  std::string m1_addr_str = f1u_head + std::to_string(sub_net) + f1u_tail;
...
f1ap_drb_to_setup srsran::make_drb_to_setup(const asn1::f1ap::drbs_to_be_setup_item_s& drb_item)
{
  ...
  for (auto uluptnl_info : drb_obj.uluptnl_info_list)
  {
    srsran::up_transport_layer_info m1_addr_info = {};
    m1_addr_info.gtp_teid = uluptnl_info.gtp_teid;
    m1_addr_info.tp_address = m1_addr_info.tp_address.create_from_string(
      derive_m1_from_f1u(uluptnl_info.tp_address));
    drb_obj.ulupm1_info_list.push_back(m1_addr_info);
  }
...
f1ap_drb_to_modify srsran::make_drb_to_modify(const asn1::f1ap::drbs_to_be_modified_item_s& drb_item)
{
  ...
  srslog::fetch_basic_logger("DU-F1").debug("Deriving M1 address from F1-U address.");
  for (auto uluptnl_info : drb_obj.uluptnl_info_list)
  {
    srsran::up_transport_layer_info m1_addr_info = {};
    m1_addr_info.gtp_teid = uluptnl_info.gtp_teid;
