...
rlc_tx_am_entity::rlc_tx_am_entity(gnb_du_id_t                          gnb_du_id,
                                  ...
                                  //  rlc_tx_m1_data_notifier&             m1_upper_dn_,
...
// void rlc_tx_am_entity::handle_m1_sdu(byte_buffer sdu_buf)
// {
//   logger.log_debug("Received MARK layer sdu {} bytes, discarding", sdu_buf.length());
// }
...
void rlc_tx_am_entity::update_mac_buffer_state(bool is_locked, bool force_notify)
{
  ...
  // // HW: also notify the CU part for l4s buffer management
  // if (rb_id.is_drb()) {
  //   mru_ul_buffer_state ul_message = {};
  //   ul_message.is_am = true; // as this is in the rlc_tx_am_entity.
  //   ul_message.drb_id = rb_id.get_drb_id();
  //   ul_message.tx_sdu = 10;
  //   ul_message.tx_bytes = 1400;
  //   ul_message.retx_sdu = 10;
  //   ul_message.retx_bytes = 1400;
  //   bool append_res = ul_message.t_pdu.append((uint8_t)3);
  //   if (append_res) {
  //     logger.log_info("ul_message t_pdu size {}", ul_message.t_pdu.length());
  //     m1_upper_dn.on_new_sdu(std::move(ul_message));
  //   }
  // } 
...
uint32_t rlc_tx_am_entity::get_buffer_state_nolock()
{
  ...
  logger.log_debug("RLC state: sdu_queue_n_sdu={}, queue_bytes={}, segment_bytes={}, retx_bytes={}, n_retx_so_zero={}, n_retx_so_nonzero={}, statues_report_bytes={}", 
                    queue_state.n_sdus, 
                    queue_bytes, 
                    segment_bytes,
                    retx_bytes,
                    retx_state.get_n_retx_so_zero(),
                    retx_state.get_n_retx_so_nonzero(),
                    status_bytes);
