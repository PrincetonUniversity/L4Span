...
public:
  index_based_ue_executor_mapper(const std::vector<task_executor*>& ctrl_execs,
                                 ...
                                 const std::vector<task_executor*>& m1_ul_execs = {}) :
    execs(ctrl_execs.size())
  {
    ...
    srsran_assert(m1_ul_execs.size() == 0 or m1_ul_execs.size() == ctrl_execs.size(),
                  "If specified, the number of UE M1 UL executors must be equal to the number of control executors");
    ...
    for (unsigned i = 0; i != ctrl_execs.size(); ++i) {
      ...
      execs[i].m1_ul_exec = m1_ul_execs.empty() ? execs[i].ul_exec : m1_ul_execs[i];
    }
  }
  ...
  task_executor& m1_ul_pdu_executor(du_ue_index_t ue_index) override
  {
    ue_index = ue_index < MAX_NOF_DU_UES ? ue_index : to_du_ue_index(0);
    return *execs[ue_index % execs.size()].m1_ul_exec;
  }
...
public:
    ...
    execs(ctrl_execs.size())
  {
    ...
    srsran_assert(dl_execs.empty() or dl_execs.size() == ctrl_execs.size(),
                  "If specified, the number of UE DL executors must be equal to the number of control executors");
    srsran_assert(m1_ul_execs.empty() or m1_ul_execs.size() == ctrl_execs.size(),
                  "If specified, the number of UE M1 UL executors must be equal to the number of control executors");
  ...
  {
    if (ue_index < MAX_NOF_DU_UES) {
      return *execs[ue_idx_to_exec_index[ue_index]].m1_ul_exec;
    }
    return *execs[0].m1_ul_exec;
  }
