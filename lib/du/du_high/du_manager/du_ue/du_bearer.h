...
struct du_drb_connector {
  ...
  // m1_rx_rlc_sdu_adapter           m1_rx_sdu_notif;
  // m1_gateway_mru_rx_adapter       m1_gateway_mru_rx_notif;
  // rlc_tx_m1_sdu_adapter           rlc_tx_m1_sdu_notif;

  /// \brief Connect MAC, RLC and F1AP layers if bearer is a DRB. Also M1 bearer and gateway.
  void connect(du_ue_index_t                       ue_index,
               drb_id_t                            drb_id,
               lcid_t                              lcid,
...
struct du_ue_drb {
  ...
  // std::vector<up_transport_layer_info>   ulupm1_info_list;
  // std::vector<up_transport_layer_info>   dlupm1_info_list;
  std::unique_ptr<f1u_du_gateway_bearer> f1u_gw_bearer;
  // std::unique_ptr<m1_du_gateway_bearer>  m1_gw_bearer;
  std::unique_ptr<f1u_bearer>            drb_f1u;
  // std::unique_ptr<m1_bearer>             drb_m1;
  std::unique_ptr<rlc_entity>            rlc_bearer;
...
struct drb_creation_info {
  ...
  // const m1_config&                     m1_cfg;
  span<const up_transport_layer_info>  uluptnl_info_list;
  // span<const up_transport_layer_info>  ulupm1_info_list;
