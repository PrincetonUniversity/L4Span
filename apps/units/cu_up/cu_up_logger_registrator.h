...
inline void register_cu_up_loggers(const cu_up_unit_logger_config& log_cfg)
{
  ...
  // auto& m1_logger = srslog::fetch_basic_logger("CU-M1", false);
  // m1_logger.set_level(log_cfg.m1_level);
  // m1_logger.set_hex_dump_max_size(log_cfg.hex_max_size);
  ...
  auto& mark_logger = srslog::fetch_basic_logger("MARK", false);
  mark_logger.set_level(log_cfg.mark_level);
  mark_logger.set_hex_dump_max_size(log_cfg.hex_max_size);
}
