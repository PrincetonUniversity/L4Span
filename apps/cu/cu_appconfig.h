...
/// MR-U configuration
struct cu_mru_appconfig {
  std::string bind_addr       = "127.0.11.1"; // Bind address used by the M1 interface
  std::string ext_addr        = "auto";       // External address advertised by the M1 interface
  int         udp_rx_max_msgs = 256; // Max number of UDP packets received by a single syscall on the M1 interface.
  float       pool_occupancy_threshold = 0.9; // Buffer pool occupancy threshold after which packets are dropped.
};
...
struct cu_appconfig {
  ...
  /// MR-U
  srs_cu::cu_mru_appconfig mru_cfg;
