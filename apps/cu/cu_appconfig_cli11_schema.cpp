...
static void configure_cli11_mru_args(CLI::App& app, srs_cu::cu_mru_appconfig& mru_cfg)
{
  add_option(app,
             "--bind_addr",
             mru_cfg.bind_addr,
      ...
      ->check(CLI::ValidIPV4);
  app.add_option(
      "--ext_addr", mru_cfg.ext_addr, "External IP address that is advertised to receive M1 packets from the DU");
  add_option(app, "--udp_max_rx_msgs", mru_cfg.udp_rx_max_msgs, "Maximum amount of messages RX in a single syscall");
  add_option(app,
             "--pool_threshold",
             mru_cfg.pool_occupancy_threshold,
             "Pool occupancy threshold after which packets are dropped")
      ->check(CLI::Range(0.0, 1.0));
  ;
}
...
void srsran::configure_cli11_with_cu_appconfig_schema(CLI::App& app, cu_appconfig& cu_cfg)
{
  ...
  // MR-U section
  CLI::App* mru_subcmd   = add_subcommand(*cu_up_subcmd, "mru", "MR-U parameters")->configurable();
  configure_cli11_mru_args(*mru_subcmd, cu_cfg.mru_cfg);
}
