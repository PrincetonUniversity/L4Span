...
static void configure_cli11_m1_args(CLI::App& app, srs_du::mru_appconfig& m1_params)
{
  app.add_option("--queue_size", m1_params.pdu_queue_size, "M1 PDU queue size")->capture_default_str();
  app.add_option(
         "--bind_addr", m1_params.bind_address, "DU M1 bind address. If left empty, implicit bind is performed")
      ->capture_default_str();
  app.add_option("--ext_addr",
                 m1_params.ext_addr,
                 "External IP address that is advertised to receive M1 packets from the CU-UP");
  app.add_option(
         "--pool_threshold", m1_params.pool_threshold, "Pool occupancy threshold after which packets are dropped")
      ->capture_default_str();
}
...
void srsran::configure_cli11_with_du_appconfig_schema(CLI::App& app, du_appconfig& du_cfg)
{
  ...
  // M1 section.
  CLI::App* mru_subcmd = app.add_subcommand("mru", "MR-U interface configuration")->configurable();
  configure_cli11_m1_args(*mru_subcmd, du_cfg.mru_cfg);
