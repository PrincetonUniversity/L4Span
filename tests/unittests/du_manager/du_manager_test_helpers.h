...
public:
  ...
  task_executor& m1_ul_pdu_executor(du_ue_index_t ue_index) override { return exec; }
...
class m1_gw_bearer_dummy : public m1_du_gateway_bearer
{
public:
  srs_du::m1_du_gateway_bearer_rx_notifier& du_rx;

  std::optional<mru_ul_buffer_state> last_sdu;

  m1_gw_bearer_dummy(srs_du::m1_du_gateway_bearer_rx_notifier& du_rx_) : du_rx(du_rx_) {}

  void stop() override {}

  void on_new_pdu(mru_ul_buffer_state msg) override { last_sdu = std::move(msg); }

  // helper function to push DL PDUs to the RX path.
  void on_new_sdu(mru_dl_message msg) { du_rx.on_new_pdu(std::move(msg)); }
};

class m1_gateway_dummy : public m1_du_gateway
{
public:
  bool next_bearer_is_created = true;
  ...
  std::unique_ptr<m1_du_gateway_bearer> create_du_bearer(uint32_t                                 ue_index,
                                                        ...
                                                        task_executor& ue_executor) override
  {
    if (next_bearer_is_created and m1_bearers.count(dl_up_tnl_info) == 0) {
      auto m1_bearer = std::make_unique<m1_gw_bearer_dummy>(du_rx);
      m1_bearers.insert(std::make_pair(dl_up_tnl_info, std::map<up_transport_layer_info, m1_gw_bearer_dummy*>{}));
      m1_bearers[dl_up_tnl_info].emplace(ul_up_tnl_info, m1_bearer.get());
      return m1_bearer;
    }
    return nullptr;
  }
  ...
  void remove_du_bearer(const up_transport_layer_info& dl_tnl_info) override
  {
    auto bearer_it = m1_bearers.find(dl_tnl_info);
    if (bearer_it == m1_bearers.end()) {
      srslog::fetch_basic_logger("TEST").warning("Could not find DL-TEID at DU to remove. DL-TEID={}",
                                                 dl_tnl_info.gtp_teid);
      return;
    }
    m1_bearers.erase(bearer_it);
  }
  ...
  std::map<up_transport_layer_info, std::map<up_transport_layer_info, m1_gw_bearer_dummy*>> m1_bearers;

  void set_m1_ext_addr(const std::string& addr) { m1_ext_addr = addr; }
