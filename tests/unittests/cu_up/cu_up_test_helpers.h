...
class dummy_cu_up_executor_mapper final : public srs_cu_up::cu_up_executor_mapper
{
  ...
  public:
    ...
    task_executor& ul_pdu_mark_executor() override { return *exec; }
    // task_executor& ul_m1_pdu_executor() override { return *exec; }
    // task_executor& dl_m1_pdu_executor() override { return *exec; }
...
public:
  ...
  // task_executor& io_ul_m1_executor() override { return *test_executor; }
...
public:
  ...
  std::unique_ptr<f1u_cu_up_gateway_bearer> create_cu_bearer(uint32_t                              ue_index,
                                                             ...
                                                             task_executor&                        ul_mark_exec) override
  {
...
// class dummy_inner_m1_bearer final : srs_cu_up::m1_tx_pdu_notifier
// {
// private:
//   // m1_cu_up_gateway_bearer_rx_notifier* rx_sdu_notifier = nullptr;

//   std::list<mru_dl_message> tx_sdu_list;
//   std::mutex                tx_sdu_mutex;
//   std::condition_variable   tx_sdu_cv;
...
//   dummy_inner_m1_bearer()           = default;
//   ~dummy_inner_m1_bearer() override = default;

//   void connect_m1_rx_sdu_notifier(m1_cu_up_gateway_bearer_rx_notifier& rx_sdu_notifier_)
//   {
//     rx_sdu_notifier = &rx_sdu_notifier_;
//   }

//   void handle_pdu(mru_ul_buffer_state msg)
//   {
//     // Forward T-PDU to PDCP
//     srsran_assert(rx_sdu_notifier != nullptr, "The rx_sdu_notifier must not be a nullptr!");
//     rx_sdu_notifier->on_new_pdu(std::move(msg));
...
//   // This should be handle_sdu
//   void on_new_pdu(mru_dl_message sdu) final
//   {
//     std::lock_guard<std::mutex> lock(tx_sdu_mutex);
//     tx_sdu_list.push_back(std::move(sdu));
//     tx_sdu_cv.notify_one();
...
//   mru_dl_message wait_tx_sdu()
//   {
//     std::unique_lock<std::mutex> lock(tx_sdu_mutex);
//     if (tx_sdu_cv.wait_for(lock, default_wait_timeout, [&]() { return !tx_sdu_list.empty(); })) {
//       // success
//       mru_dl_message sdu = std::move(tx_sdu_list.front());
//       tx_sdu_list.pop_front();
//       return sdu;
//     }
//     // timeout
...
// class dummy_m1_gateway_bearer final : public m1_cu_up_gateway_bearer
// {
// public:
//   dummy_m1_gateway_bearer(dummy_inner_m1_bearer&             inner_,
...
//   ~dummy_m1_gateway_bearer() override { stop(); }
...
//   void connect_m1_rx_sdu_notifier(m1_cu_up_gateway_bearer_rx_notifier& rx_sdu_notifier_)
//   {
//     inner.connect_m1_rx_sdu_notifier(rx_sdu_notifier_);
//   }

//   void on_new_pdu(mru_dl_message sdu) final { inner.on_new_pdu(std::move(sdu)); }

// private:
//   bool                                stopped = false;
//   dummy_inner_m1_bearer&             inner;
//   srs_cu_up::m1_bearer_disconnector& disconnector;
//   up_transport_layer_info             ul_up_tnl_info;
// };

// class dummy_m1_gateway final : public m1_cu_up_gateway
// {
// private:
//   dummy_inner_m1_bearer& bearer;

// public:
//   explicit dummy_m1_gateway(dummy_inner_m1_bearer& bearer_) : bearer(bearer_) {}
//   ~dummy_m1_gateway() override = default;
...
//   {
//     created_ul_teid_list.push_back(ul_up_tnl_info.gtp_teid);
//     bearer.connect_m1_rx_sdu_notifier(rx_notifier);
//     return std::make_unique<dummy_m1_gateway_bearer>(bearer, *this, ul_up_tnl_info);
//   }
